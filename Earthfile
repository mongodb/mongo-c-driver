...
#   A build environment based on Alpine Linux version 3.19
alpine3.19-build-env:
...
multibuild:
    BUILD +test-example --env=ubuntu2204 --env=archlinux --env=alpine3.19 \
            --sasl=cyrus --sasl=off \
            --tls=openssl --tls=off
    ...
    BUILD +test-example --env=archlinux --env=alpine3.19 \
            --sasl=cyrus --sasl=off \
            --tls=libressl
