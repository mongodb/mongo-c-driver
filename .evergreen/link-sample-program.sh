...
if [ "$OS" = "darwin" ]; then
  ZSTD_CMAKE_OPTION="-DENABLE_ZSTD=OFF"
fi


$CMAKE -DCMAKE_INSTALL_PREFIX=$INSTALL_DIR -DCMAKE_PREFIX_PATH=$INSTALL_DIR/lib/cmake $SSL_CMAKE_OPTION $SNAPPY_CMAKE_OPTION $STATIC_CMAKE_OPTION -DENABLE_BSON=ON $ZSTD_CMAKE_OPTION .
make
make install
