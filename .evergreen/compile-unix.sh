...
if [ -e pipe ]; then
   set +o xtrace
   tee error.log < pipe &
   run_valgrind ./src/libmongoc/test-libmongoc -d --match "/atomic/*" -F test-results.json 2>pipe
   rm pipe
else
   run_valgrind ./src/libmongoc/test-libmongoc -d --match "/atomic/*" -F test-results.json
fi
