...
_mongoc_cyrus_start (mongoc_cyrus_t *sasl, uint8_t **outbuf, uint32_t *outbuflen, bson_error_t *error)
{
   ...
   *outbuf = bson_malloc (outbuf_capacity);
   ...
   b64_ret = mcommon_b64_ntop ((uint8_t *) raw, raw_len, (char *) *outbuf, outbuf_capacity);
   if (b64_ret == -1) {
      bson_set_error (
                    ...
                    bson_error_t *error)
{
   ...
   b64_ret = mcommon_b64_pton ((char *) inbuf, (uint8_t *) decoded, decoded_capacity);
   if (b64_ret == -1) {
      bson_set_error (
   ...
   *outbuf = bson_malloc0 (outbuf_capacity);
   b64_ret = mcommon_b64_ntop ((const uint8_t *) raw, rawlen, (char *) *outbuf, outbuf_capacity);
   if (b64_ret == -1) {
      bson_set_error (
         error, MONGOC_ERROR_SASL, MONGOC_ERROR_CLIENT_AUTHENTICATE, "Unable to base64 encode client SASL message");
      bson_free (decoded);
