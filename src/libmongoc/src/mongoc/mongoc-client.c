                      ...
                      bson_error_t *error)
{
   ...
   for (i = 0; i < n; i++) {
      if (ns_parserr(&ns_answer, ns_s_an, i, &resource_record)) {
         DNS_ERROR(
            "Invalid record %d of %s answer for \"%s\": \"%s\"", i, rr_type_name, hostname, _mongoc_hstrerror(h_errno));
      }
                      ...
                      bson_error_t *error)
{
   ...
   connecttimeoutms = mongoc_uri_get_option_as_int32(uri, MONGOC_URI_CONNECTTIMEOUTMS, MONGOC_DEFAULT_CONNECTTIMEOUTMS);
...
mongoc_client_set_read_prefs(mongoc_client_t *client, const mongoc_read_prefs_t *read_prefs)
{
   ...
   if (read_prefs != client->read_prefs) {
      ...
      client->read_prefs = read_prefs ? mongoc_read_prefs_copy(read_prefs) : mongoc_read_prefs_new(MONGOC_READ_PRIMARY);
   }
}
...
mongoc_client_get_server_descriptions(const mongoc_client_t *client, size_t *n /* OUT */)
{
   ...
   mongoc_server_description_t **const sds = mongoc_topology_description_get_servers(td.ptr, BSON_ASSERT_PTR_INLINE(n));
   mc_tpld_drop_ref(&td);
   return sds;
}
