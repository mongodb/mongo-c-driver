   ...
   mongoc_cluster_t *cluster, mongoc_cmd_t *cmd, int32_t compressor_id, bson_t *reply, bson_error_t *error)
{
   ...
   if (compressor_id != -1 && IS_NOT_COMMAND (HANDSHAKE_CMD_LEGACY_HELLO) && IS_NOT_COMMAND ("hello") &&
    ...
    * send and receive
    */
   if (!_mongoc_stream_writev_full (stream, cluster->iov.data, cluster->iov.len, cluster->sockettimeoutms, error)) {
      _handle_network_error (cluster, cmd->server_stream, true /* handshake complete */, error);
   ...
   if (reply_header_size !=
       mongoc_stream_read (stream, &reply_header_buf, reply_header_size, reply_header_size, cluster->sockettimeoutms)) {
   ...
   if (!_mongoc_rpc_scatter_reply_header_only (&rpc, reply_header_buf, reply_header_size)) {
      _handle_network_error (cluster, cmd->server_stream, true /* handshake complete */, error);
      GOTO (done);
   }
   ...
   if (BSON_UINT32_FROM_LE (rpc.header.opcode) == MONGOC_OPCODE_COMPRESSED) {
      ...
      uint8_t *buf = NULL;
      size_t len = BSON_UINT32_FROM_LE (rpc.compressed.uncompressed_size) + sizeof (mongoc_rpc_header_t);

      reply_buf = bson_malloc0 (msg_len);
      memcpy (reply_buf, reply_header_buf, reply_header_size);

      if (doc_len !=
          mongoc_stream_read (stream, reply_buf + reply_header_size, doc_len, doc_len, cluster->sockettimeoutms)) {
         RUN_CMD_ERR (MONGOC_ERROR_STREAM, MONGOC_ERROR_STREAM_SOCKET, "socket error or timeout");
         _handle_network_error (cluster, cmd->server_stream, true /* handshake complete */, error);
         GOTO (done);
      }
      ...
      if (!_mongoc_rpc_decompress (&rpc, buf, len)) {
         RUN_CMD_ERR (MONGOC_ERROR_PROTOCOL, MONGOC_ERROR_PROTOCOL_INVALID_REPLY, "Could not decompress server reply");
         bson_free (reply_buf);
         bson_free (buf);
         GOTO (done);
      }
      ...
      if (doc_len != mongoc_stream_read (stream, (void *) reply_buf, doc_len, doc_len, cluster->sockettimeoutms)) {
         RUN_CMD_ERR (MONGOC_ERROR_STREAM, MONGOC_ERROR_STREAM_SOCKET, "socket error or timeout");
         _handle_network_error (cluster, cmd->server_stream, true /* handshake complete */, error);
         GOTO (done);
      }
   ...
   if (!_mongoc_cmd_check_ok (reply_ptr, cluster->client->error_api_version, error)) {
      GOTO (done);
   }
...
_handle_txn_error_labels (bool cmd_ret, const bson_error_t *cmd_err, const mongoc_cmd_t *cmd, bson_t *reply)
{
   ...
   _mongoc_write_error_handle_labels (cmd_ret, cmd_err, reply, cmd->server_stream->sd->max_wire_version);
}
...
mongoc_cluster_run_command_monitored (mongoc_cluster_t *cluster, mongoc_cmd_t *cmd, bson_t *reply, bson_error_t *error)
{
   ...
   if (mongoc_cluster_uses_server_api (cluster) || server_stream->sd->max_wire_version >= WIRE_VERSION_OP_MSG) {
      retval = mongoc_cluster_run_opmsg (cluster, cmd, reply, error);
   } else {
      retval = mongoc_cluster_run_command_opquery (cluster, cmd, compressor_id, reply, error);
   }
   ...
   if (retval && _in_sharded_txn (cmd->session) && bson_iter_init_find (&iter, reply, "recoveryToken")) {
                   ...
                   bson_error_t *error /* OUT */)
{
   ...
   handshake_sd = _cluster_run_hello (
      cluster, cluster_node, server_id, cluster->scram_cache, &scram, &speculative_auth_response, error);
   if (!handshake_sd) {
      GOTO (error);
   }
                                  ...
                                  bson_error_t *error)
{
   ...
   server_stream = _mongoc_cluster_stream_for_server (cluster, server_id, reconnect_ok, cs, reply, error);
                                  ...
                                  bson_error_t *error)
{
   ...
   if (_in_sharded_txn (cs)) {
      ...
      if (!server_id) {
         server_id = mongoc_topology_select_server_id (topology, optype, read_prefs, must_use_primary, error);
         if (server_id) {
            _mongoc_client_session_pin (cs, server_id);
         }
      }
                                   ...
                                   bson_error_t *error)
{
   ...
   server_id = _mongoc_cluster_select_server_id (cs, topology, optype, read_prefs, &must_use_primary, error);
   ...
   server_stream = _mongoc_cluster_stream_for_server (cluster, server_id, true /* reconnect_ok */, cs, reply, error);
   if (server_stream) {
...
mongoc_cluster_stream_for_writes (mongoc_cluster_t *cluster,
                                  ...
                                  bson_error_t *error)
{
   return _mongoc_cluster_stream_for_optype (cluster, MONGOC_SS_WRITE, NULL, cs, reply, error);
}
                                           ...
                                           bson_error_t *error)
{
   ...
   if (!_mongoc_stream_writev_full (
          server_stream->stream, cluster->iov.data, cluster->iov.len, cluster->sockettimeoutms, error)) {
      GOTO (done);
   }
                         ...
                         bson_error_t *error)
{
    ...
    */
   pos = buffer->len;
   if (!_mongoc_buffer_append_from_stream (buffer, server_stream->stream, 4, cluster->sockettimeoutms, error)) {
      MONGOC_DEBUG ("Could not read 4 bytes, stream probably closed or timed out");
      mongoc_counter_protocol_ingress_error_inc ();
      _handle_network_error (cluster, server_stream, true /* handshake complete */, error);
      RETURN (false);
   }
   ...
   if ((msg_len < 16) || (msg_len > max_msg_size)) {
      bson_set_error (
         error, MONGOC_ERROR_PROTOCOL, MONGOC_ERROR_PROTOCOL_INVALID_REPLY, "Corrupt or malicious reply received.");
      _handle_network_error (cluster, server_stream, true /* handshake complete */, error);
      mongoc_counter_protocol_ingress_error_inc ();
      RETURN (false);
   }
   ...
   if (!_mongoc_buffer_append_from_stream (
          buffer, server_stream->stream, msg_len - 4, cluster->sockettimeoutms, error)) {
      _handle_network_error (cluster, server_stream, true /* handshake complete */, error);
      mongoc_counter_protocol_ingress_error_inc ();
      RETURN (false);
   }
...
mongoc_cluster_run_opmsg (mongoc_cluster_t *cluster, mongoc_cmd_t *cmd, bson_t *reply, bson_error_t *error)
{
   ...
   if (cmd->payload) {
      ...
      section[1].payload.sequence.size = cmd->payload_size + strlen (cmd->payload_identifier) + 1 + sizeof (int32_t);
   ...
   if (mongoc_cmd_is_compressible (cmd)) {
      int32_t compressor_id = mongoc_server_description_compressor_id (server_stream->sd);
   ...
   ok = _mongoc_stream_writev_full (
      server_stream->stream, (mongoc_iovec_t *) cluster->iov.data, cluster->iov.len, cluster->sockettimeoutms, error);
   if (!ok) {
      /* add info about the command to writev_full's error message */
      RUN_CMD_ERR_DECORATE;
      _handle_network_error (cluster, server_stream, true /* handshake complete */, error);
   ...
   if (cmd->is_acknowledged) {
      ok = _mongoc_buffer_append_from_stream (&buffer, server_stream->stream, 4, cluster->sockettimeoutms, error);
      if (!ok) {
         RUN_CMD_ERR_DECORATE;
         _handle_network_error (cluster, server_stream, true /* handshake complete */, error);
         server_stream->stream = NULL;
      ...
      ok = _mongoc_buffer_append_from_stream (
         &buffer, server_stream->stream, (size_t) msg_len - 4, cluster->sockettimeoutms, error);
      if (!ok) {
         RUN_CMD_ERR_DECORATE;
         _handle_network_error (cluster, server_stream, true /* handshake complete */, error);
         server_stream->stream = NULL;
      ...
      if (BSON_UINT32_FROM_LE (rpc.header.opcode) == MONGOC_OPCODE_COMPRESSED) {
         ...
         if (!_mongoc_rpc_decompress (&rpc, (uint8_t *) output, len)) {
            RUN_CMD_ERR (
               MONGOC_ERROR_PROTOCOL, MONGOC_ERROR_PROTOCOL_INVALID_REPLY, "Could not decompress message from server");
            _handle_network_error (cluster, server_stream, true /* handshake complete */, error);
            server_stream->stream = NULL;
            bson_free (output);
            network_error_reply (reply, cmd);
      ...
      _mongoc_topology_update_cluster_time (cluster->client->topology, &reply_local);
      ok = _mongoc_cmd_check_ok (&reply_local, cluster->client->error_api_version, error);

      if (cmd->session) {
         _mongoc_client_session_handle_reply (cmd->session, cmd->is_acknowledged, cmd->command_name, &reply_local);
      }
