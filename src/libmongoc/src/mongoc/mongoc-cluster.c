   ...
   mongoc_cluster_t *cluster, const mongoc_cmd_t *cmd, int32_t compressor_id, mcd_rpc_message *rpc, bson_error_t *error)
{
   ...
   if (!_mongoc_stream_writev_full_with_socket_timeout_convention(
...
run_command_monitored(mongoc_cluster_t *cluster, mongoc_cmd_t *cmd, bson_t *reply, bson_error_t *error)
{
   ...
   if (retval && _in_sharded_or_loadbalanced_txn(cmd->session) && bson_iter_init_find(&iter, reply, "recoveryToken")) {
                                ...
                                bson_error_t *error)
{
   ...
   if (buflen == -1) {
      _mongoc_set_error(error, MONGOC_ERROR_CLIENT, MONGOC_ERROR_CLIENT_AUTHENTICATE, "failed base64 encoding message");
      return false;
   }
                                        ...
                                        bson_error_t *error)
{
   ...
   if (strcasecmp(mechanism, "SCRAM-SHA-1") == 0 || strcasecmp(mechanism, "SCRAM-SHA-256") == 0) {
      ...
      ret = _mongoc_cluster_auth_scram_continue(cluster, stream, handshake_sd, scram, speculative_auth_response, error);
   }
#endif
                  ...
                  bson_error_t *error /* OUT */)
{
    ...
    * description */
   handshake_sd->generation = _mongoc_topology_get_connection_pool_generation(td, server_id, &handshake_sd->service_id);
                                  ...
                                  bson_error_t *error)
{
   ...
   if (is_retryable && retryable_error) {
      ...
      server_stream =
         _mongoc_cluster_stream_for_server(cluster, server_id, true /* reconnect_ok */, cs, &retry_reply, &retry_error);
...
mongoc_cluster_get_max_msg_size(mongoc_cluster_t *cluster)
{
   ...
   if (!cluster->client->topology->single_threaded) {
      ...
      mc_shared_tpld td = mc_tpld_take_ref(BSON_ASSERT_PTR_INLINE(cluster)->client->topology);
      mongoc_set_for_each_const(mc_tpld_servers_const(td.ptr), _mongoc_cluster_min_of_max_msg_size_sds, &max_msg_size);
      mc_tpld_drop_ref(&td);
   }
                                          ...
                                          bson_error_t *error)
{
   ...
   mcd_rpc_message_egress(rpc);
   if (!_mongoc_stream_writev_full_with_socket_timeout_convention(
          server_stream->stream, iovecs, num_iovecs, cluster->sockettimeoutms, error)) {
      GOTO(done);
   }
   ...
   mongoc_cluster_t *cluster, const mongoc_cmd_t *cmd, mcd_rpc_message *rpc, bson_t *reply, bson_error_t *error)
{
   ...
   mcd_rpc_message_egress(rpc);
   const bool res = _mongoc_stream_writev_full_with_socket_timeout_convention(
      server_stream->stream, iovecs, num_iovecs, cluster->sockettimeoutms, error);
