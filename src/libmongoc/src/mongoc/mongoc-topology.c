...
_mongoc_topology_scanner_setup_err_cb (uint32_t id, void *data, const bson_error_t *error /* IN */)
{
   ...
   if (_mongoc_topology_get_type (topology) == MONGOC_TOPOLOGY_LOAD_BALANCED) {
      ...
      mongoc_topology_description_handle_hello (mod.new_td, id, NULL /* hello reply */, -1 /* rtt_msec */, error);
   ...
   uint32_t id, const bson_t *hello_response, int64_t rtt_msec, void *data, const bson_error_t *error /* IN */)
{
   ...
   if (!hello_response) {
       ...
       * pool for the server */
      _mongoc_topology_description_clear_connection_pool (tdmod.new_td, id, &kZeroServiceId);
   }
   ...
   if (!hello_response && sd && sd->type != MONGOC_SERVER_UNKNOWN) {
      _mongoc_topology_update_no_lock (id, hello_response, rtt_msec, tdmod.new_td, error);
...
_mongoc_apply_srv_max_hosts (const mongoc_host_list_t *hl, const int32_t max_hosts, size_t *const hl_array_size)
{
   size_t hl_size;
   size_t idx;
...
mongoc_topology_new (const mongoc_uri_t *uri, bool single_threaded)
{
...

   {
      ...
      const mongoc_host_list_t *const *hl_array = _mongoc_apply_srv_max_hosts (hl, td->max_hosts, &hl_array_size);
                            ...
                            bson_error_t *error)
{
   ...
   if (max_staleness_seconds != MONGOC_NO_MAX_STALENESS) {
      max_wire_version = mongoc_topology_description_lowest_max_wire_version (td);
...
mongoc_topology_select (mongoc_topology_t *topology,
                        ...
                        bson_error_t *error)
{
   uint32_t server_id = mongoc_topology_select_server_id (topology, optype, read_prefs, must_use_primary, error);
...
_mongoc_topology_select_server_id_loadbalanced (mongoc_topology_t *topology, bson_error_t *error)
{
   ...
   selected_server = mongoc_topology_description_select (
      td.ptr, MONGOC_SS_WRITE, NULL /* read prefs */, NULL /* chosen read mode */, 0 /* local threshold */);
                                  ...
                                  bson_error_t *error)
{
   ...
   if (topology->single_threaded) {
      ...
      for (;;) {
         ...
         selected_server =
            mongoc_topology_description_select (td.ptr, optype, read_prefs, must_use_primary, local_threshold_ms);
   ...
   if (!loadbalanced && timeout == MONGOC_NO_SESSIONS) {
      ...
      if (!mongoc_topology_description_has_data_node (td.ptr)) {
         if (!mongoc_topology_select_server_id (
                topology, MONGOC_SS_READ, NULL /* read prefs */, NULL /* chosen read mode */, error)) {
            ss = NULL;
            goto done;
         }
...
mc_tpld_modify_begin (mongoc_topology_t *tpl)
{
   ...
   new_td = mongoc_topology_description_new_copy (prev_td.ptr), mc_tpld_drop_ref (&prev_td);
