                                ...
                                const bson_error_t *error /* IN */)
{
   mongoc_topology_description_handle_hello(
      td, log_and_monitor, id, hello_response, rtt_msec, update_cluster_time, error);
   ...
   uint32_t id, const bson_t *hello_response, int64_t rtt_msec, void *data, const bson_error_t *error /* IN */)
{
   ...
   if (!hello_response && sd && sd->type != MONGOC_SERVER_UNKNOWN) {
      _mongoc_topology_update_no_lock(
         id, hello_response, rtt_msec, /*update_cluster_time=*/false, td, &topology->log_and_monitor, error);
...
mongoc_topology_new(const mongoc_uri_t *uri, bool single_threaded)
{
   ...
   if (single_threaded) {
       ...
       */
      topology->server_selection_try_once = mongoc_uri_get_option_as_bool(uri, MONGOC_URI_SERVERSELECTIONTRYONCE, true);
   } else {
...
mongoc_topology_rescan_srv(mongoc_topology_t *topology)
{
   ...
   /* Go forth and query... */
   prefixed_hostname = bson_strdup_printf("_%s._tcp.%s", mongoc_uri_get_srv_service_name(topology->uri), srv_hostname);
                                 ...
                                 bson_error_t *error)
{
   ...
   if (server_id == 0) {
      ...
      mongoc_structured_log(log_and_monitor->structured_log,
                            ...
                            int64(log_context->has_operation_id ? "operationId" : NULL, log_context->operation_id),
                            topology_as_description_json("topologyDescription", topology),
                            server_description(mongoc_topology_description_server_by_id_const(td.ptr, server_id, NULL),
                                               SERVER_HOST,
                                               SERVER_PORT));
   }
