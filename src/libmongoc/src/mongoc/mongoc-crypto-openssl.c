                                        ...
                                        unsigned char *output)
{
   ...
   if (BSON_UNLIKELY (mcd_cmp_greater_us (password_len, INT_MAX))) {
      MONGOC_ERROR ("PBKDF2 HMAC password length exceeds INT_MAX");
      return false;
   }

   if (BSON_UNLIKELY (mcd_cmp_greater_us (salt_len, INT_MAX))) {
      MONGOC_ERROR ("PBKDF2 HMAC salt length exceeds INT_MAX");
      return false;
   }

   if (BSON_UNLIKELY (mcd_cmp_greater_us (iterations, INT_MAX))) {
