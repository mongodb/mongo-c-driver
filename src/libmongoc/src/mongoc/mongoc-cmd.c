                                           ...
                                           mongoc_logged_cmd_content_flags_t flags)
{
   ...
   if ((flags & MONGOC_LOGGED_CMD_CONTENT_FLAG_DATABASE_NAME) &&
       !BSON_APPEND_UTF8 (bson, "databaseName", cmd->db_name)) {
      return false;
   }
   if ((flags & MONGOC_LOGGED_CMD_CONTENT_FLAG_COMMAND_NAME) &&
       !BSON_APPEND_UTF8 (bson, "commandName", cmd->command_name)) {
      return false;
   }
   if ((flags & MONGOC_LOGGED_CMD_CONTENT_FLAG_OPERATION_ID) &&
       !BSON_APPEND_INT64 (bson, "operationId", cmd->operation_id)) {
      return false;
   }
   if (flags & MONGOC_LOGGED_CMD_CONTENT_FLAG_COMMAND) {
      if (mongoc_apm_is_sensitive_command_message (cmd->command_name, cmd->command)) {
         if (!BSON_APPEND_UTF8 (bson, "command", "{}")) {
            return false;
         }
      } else {
         ...
         if (json) {
            ...
            ok = bson_append_utf8 (bson, key, strlen (key), json, json_length);
            bson_free (json);
         }
