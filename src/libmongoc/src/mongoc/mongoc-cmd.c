...
mongoc_cmd_parts_append_opts (mongoc_cmd_parts_t *parts, bson_iter_t *iter, int max_wire_version, bson_error_t *error)
{
   ...
   while (bson_iter_next (iter)) {
      if (BSON_ITER_IS_KEY (iter, "collation")) {
         ...
         if (!mongoc_cmd_parts_set_write_concern (parts, wc, max_wire_version, error)) {
                                    ...
                                    bson_error_t *error)
{
   ...
   wc_allowed = parts->is_write_command || (is_fam && max_wire_version >= WIRE_VERSION_FAM_WRITE_CONCERN) ||
                                    ...
                                    bson_error_t *error)
{
   ...
   if (!bson_empty (&rw_opts->collation)) {
      if (max_wire_version < WIRE_VERSION_COLLATION) {
         OPTS_ERR (PROTOCOL_BAD_WIRE_VERSION, "The selected server does not support collation");
      }
   ...
   if (!mongoc_cmd_parts_set_write_concern (parts, rw_opts->writeConcern, max_wire_version, error)) {
      RETURN (false);
   }
   ...
   if (!bson_empty (&rw_opts->readConcern)) {
      if (max_wire_version < WIRE_VERSION_READ_CONCERN) {
         OPTS_ERR (PROTOCOL_BAD_WIRE_VERSION, "The selected server does not support readConcern");
      }
