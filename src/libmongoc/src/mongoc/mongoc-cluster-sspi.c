...
_mongoc_cluster_sspi_new(mongoc_uri_t *uri, mongoc_stream_t *stream, const char *hostname)
{
   ...
   if (state->sasl.canonicalize_host_name && _mongoc_sasl_get_canonicalized_name(stream, real_name, sizeof real_name)) {
      hostname = real_name;
   }
   ...
   res =
      _mongoc_sspi_auth_sspi_client_init(service, flags, user, (ULONG)user_len, NULL, 0, pass, (ULONG)pass_len, state);
                               ...
                               bson_error_t *error)
{
   ...
   for (step = 0;; step++) {
      ...
      if (res == MONGOC_SSPI_AUTH_GSS_ERROR) {
         _mongoc_set_error(error, MONGOC_ERROR_CLIENT, MONGOC_ERROR_CLIENT_AUTHENTICATE, "Received invalid SSPI data.");
