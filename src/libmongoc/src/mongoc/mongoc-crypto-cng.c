...
{
   if (BSON_UNLIKELY (mcd_cmp_greater_uu (password_len, ULONG_MAX))) {
      MONGOC_ERROR ("PBDKF2 HMAC password length exceeds ULONG_MAX");
      return false;
   }

   if (BSON_UNLIKELY (mcd_cmp_greater_uu (salt_len, ULONG_MAX))) {
   ...
   if (BSON_UNLIKELY (mcd_cmp_greater_uu (output_len, ULONG_MAX))) {
