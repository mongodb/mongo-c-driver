   ...
   mlib_foreach_irange (i, num_chars) {
      if (_mongoc_utf8_code_point_is_in_table(
          ...
          _mongoc_utf8_code_point_is_in_table(
             utf8_codepoints[i], unassigned_codepoint_ranges, sizeof(unassigned_codepoint_ranges) / sizeof(uint32_t))) {
         bson_free(out_utf8);
         bson_free(utf8_pre_norm);
         bson_free(utf8_codepoints);
   ...
   if (
      ...
      (contains_RandALCar &&
       (!_mongoc_utf8_code_point_is_in_table(
        ...
        !_mongoc_utf8_code_point_is_in_table(
           utf8_codepoints[num_chars - 1], RandALCat_bidi_ranges, sizeof(RandALCat_bidi_ranges) / sizeof(uint32_t))))) {
      bson_free(out_utf8);
      bson_free(utf8_pre_norm);
      bson_free(utf8_codepoints);
