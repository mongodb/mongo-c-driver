...
_mongoc_stream_tls_openssl_write (mongoc_stream_tls_t *tls, char *buf, size_t buf_len)
{
   ...
   BSON_ASSERT (mcd_in_range_unsigned (int, buf_len));
   ...
   if (expire) {
      ...
      if ((expire - now) < 0) {
         if (mcd_cmp_less_su (ret, buf_len)) {
   ...
   for (i = 0; i < iovcnt; i++) {
      ...
      while (iov_pos < iov[i].iov_len) {
         ...
         if (to_write) {
            ...
            if (mcd_cmp_not_equal_su (child_ret, to_write_len)) {
            ...
            if (mcd_cmp_less_su (child_ret, to_write_len)) {
