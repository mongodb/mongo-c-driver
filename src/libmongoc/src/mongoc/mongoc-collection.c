                                          ...
                                          mongoc_write_result_t *result)
{
   ...
   server_stream = mongoc_cluster_stream_for_writes (&collection->client->cluster, cs, NULL, &result->error);
                                              ...
                                              mongoc_write_result_t *result)
{
   ...
   server_stream =
      mongoc_cluster_stream_for_writes (&collection->client->cluster, crud->client_session, &reply, &result->error);
 ...
 */
static void
_make_aggregate_for_count (const mongoc_collection_t *coll, const bson_t *filter, const bson_t *opts, bson_t *out)
{
   bson_iter_t iter;
   bson_t pipeline;
   bson_t match_stage;
                                     ...
                                     bson_error_t *error)
{
   ...
   /* Drop ECC collection. */
   eccName = _mongoc_get_encryptedField_state_collection (encryptedFields, name, "ecc", error);
   if (!eccName) {
      goto fail;
   }

   eccCollection = mongoc_client_get_collection (collection->client, collection->db, eccName);
...
mongoc_collection_drop_with_opts (mongoc_collection_t *collection, const bson_t *opts, bson_error_t *error)
{
   ...
   if (bson_empty (&encryptedFields) && collection->client->topology->encrypted_fields_map != NULL) {
   ...
   if (!bson_empty (&encryptedFields)) {
      ...
      bool ret =
         drop_with_opts_with_encryptedFields (collection, &opts_without_encryptedFields, &encryptedFields, error);
                                          ...
                                          bson_error_t *error)
{
   ...
   server_stream = mongoc_cluster_stream_for_writes (&collection->client->cluster, parsed.client_session, reply, error);
   ...
   if (!mongoc_cmd_parts_set_write_concern (&parts, parsed.writeConcern, server_stream->sd->max_wire_version, error)) {
      GOTO (done);
   }
                                      ...
                                      bson_error_t *error)
{
   ...
   server_stream =
      mongoc_cluster_stream_for_writes (&collection->client->cluster, update_opts->crud.client_session, reply, error);
                                             ...
                                             bson_error_t *error)
{
   ...
   server_stream = mongoc_cluster_stream_for_writes (cluster, appended_opts.client_session, &ss_reply, error);
   ...
   else if (server_stream->sd->max_wire_version >= WIRE_VERSION_FAM_WRITE_CONCERN &&
   ...
   if (bson_iter_init (&iter, &appended_opts.extra)) {
      bool ok = mongoc_cmd_parts_append_opts (&parts, &iter, server_stream->sd->max_wire_version, error);
      if (!ok) {
         GOTO (done);
      }
   }
...
retry:
   ...
   if (parts.is_retryable_write) {
      _mongoc_write_error_handle_labels (ret, error, reply_ptr, server_stream->sd->max_wire_version);
   }
   ...
   if (is_retryable && _mongoc_write_error_get_type (reply_ptr) == MONGOC_WRITE_ERR_RETRY) {
      ...
      is_retryable = false;
      retry_server_stream =
         mongoc_cluster_stream_for_writes (cluster, parts.assembled.session, NULL /* reply */, &ignored_error);
