                                        ...
                                        bson_error_t *error)
{
   ...
   if (node->successful_dns_result) {
      _begin_hello_cmd (node, NULL, false, node->successful_dns_result, 0, true /* use_handshake */);
   } else {
      LL_FOREACH2 (node->dns_results, iter, ai_next)
      {
         _begin_hello_cmd (node, NULL, false, iter, delay, true /* use_handshake */);
                                    ...
                                    bson_error_t *error)
{
   ...
   if (node->stream) {
      _begin_hello_cmd (node, node->stream, true /* is_setup_done */, NULL, 0, false /* use_handshake */);
      node->stream = NULL;
      return;
   }
   ...
   if (node->ts->initiator) {
      ...
      if (stream) {
         success = true;
         _begin_hello_cmd (node, stream, false, NULL, 0, true /* use_handshake */);
      }
   } else {
