...
_mongoc_topology_scanner_dup_handshake_cmd (mongoc_topology_scanner_t *ts, bson_t *copy_into)
{
   ...
   appname = mcd_atomic_ptr_fetch ((void *) &ts->appname, mcommon_memory_order_relaxed);
...
_mongoc_topology_scanner_set_appname (mongoc_topology_scanner_t *ts, const char *appname)
{
   ...
   prev = mcd_atomic_ptr_compare_exchange_strong ((void *) &ts->appname, NULL, s, mcommon_memory_order_relaxed);
   if (prev == NULL) {
      return true;
   }
