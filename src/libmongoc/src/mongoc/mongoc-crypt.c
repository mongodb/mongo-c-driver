...
_state_need_kms_credentials (_state_machine_t *sm, bson_error_t *error)
{
   ...
   if (!callback_provided_aws && _needs_on_demand_aws_kms (&sm->crypt->kms_providers, error)) {
                                ...
                                bson_error_t *error)
{
   ...
   if (!bson_iter_init_find (&iter, &result, "v")) {
      bson_set_error (
         error, MONGOC_ERROR_CLIENT, MONGOC_ERROR_CLIENT_INVALID_ENCRYPTION_STATE, "encrypted result unexpected");
      goto fail;
   } else {
