...
fail:
   if (prev_cse_state == MONGOC_CSE_DISABLED) {
      ...
      mcd_atomic_int_exchange ((int *) &topology->cse_state, new_state, mcommon_memory_order_release);
   }
   mongoc_uri_destroy (mongocryptd_uri);
   RETURN (setup_okay);
}
...
_mongoc_cse_is_enabled (mongoc_client_t *client)
{
   ...
   while (1) {
      mongoc_topology_cse_state_t state =
         mcd_atomic_int_fetch ((int *) &client->topology->cse_state, mcommon_memory_order_relaxed);
