   ...
   while (bson_iter_next (&iter)) {
      ...
      if (!strcmp (first_key (&event), "command_started_event")) {
         ...
         if (first_operation_id == -1) {
            ...
            MONGOC_ERROR ("%s sent wrong operation_id",
...
test_get_error (void)
{
   ...
   mock_server_replies_simple (request, "{'ok': 0, 'errmsg': 'foo', 'code': 42}");
...
_test_query_operation_id (bool pooled)
{
   ...
   mock_server_replies_to_find (request, MONGOC_QUERY_NONE, 123 /* cursor id */, 1, "db.collection", "{}", true);
   ...
   future = future_cursor_next (cursor, &doc);
   request = mock_server_receives_request (server);
   mock_server_replies_simple (request, "{'ok': 0, 'code': 42, 'errmsg': 'bad!'}");
...
_test_service_id (bool is_loadbalanced)
{
   ...
   request = mock_server_receives_msg (server, 0, tmp_bson ("{'ping': 1}"));
   mock_server_replies_simple (request, "{'ok': 0, 'code': 8, 'errmsg': 'UnknownError'}");
   request_destroy (request);
