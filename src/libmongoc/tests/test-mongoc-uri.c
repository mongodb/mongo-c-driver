...
test_mongoc_uri_new(void)
{
   ...
   ASSERT(!mongoc_uri_new("mongodb://user%40DOMAIN.COM:password@localhost/"
                          ...
                          "=SERVICE_NAME:\x80"));
   ASSERT(!mongoc_uri_new("mongodb://user%40DOMAIN.COM:password@localhost/"
                          "?" MONGOC_URI_AUTHMECHANISM "=GSSAPI&" MONGOC_URI_AUTHMECHANISMPROPERTIES "=\x80:mongodb"));
   ASSERT(!mongoc_uri_new("mongodb://::"));
...
_auth_mechanism_username_required(const char *mechanism)
{
   // None.
   {
      ...
      mongoc_uri_t *const uri =
         mongoc_uri_new_with_error(tmp_str("mongodb://localhost/?" MONGOC_URI_AUTHMECHANISM "=%s", mechanism), &error);
...
test_mongoc_uri_auth_mechanisms(void)
{
   ...
   // Default Authentication Mechanism
   {
      ...
      {
         bson_error_t error;
         mongoc_uri_t *const uri = mongoc_uri_new_with_error("mongodb://localhost/?" MONGOC_URI_AUTHSOURCE "=", &error);
         ASSERT_NO_CAPTURED_LOGS("mongoc_uri_new_with_error");
         ASSERT(!uri);
         ASSERT_ERROR_CONTAINS(error,
                               MONGOC_ERROR_COMMAND,
...
test_mongoc_uri_compressors(void)
{
   ...
   ASSERT_MATCH(mongoc_uri_get_compressors(uri), "{'snappy': {'$exists': true}, 'somethingElse': {'$exists': false}}");
   ASSERT_CAPTURED_LOG(
...
test_mongoc_uri_socket_timeout_ms(void)
{
   ...
   ASSERT_CMPINT32(mongoc_uri_get_socket_timeout_ms_option(uri), ==, 0);
