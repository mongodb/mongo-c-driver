...
test_exhaust_cursor(bool pooled)
{
   ...
   /* bulk insert some records to work on */
   {
      ...
      ASSERT_OR_PRINT(mongoc_collection_insert_many(collection, (const bson_t **)bptr, 10, &opts, NULL, &error), error);
      bson_destroy(&opts);
   }
...
test_exhaust_cursor_works(void *context)
{
   ...
   // With OP_MSG, a cursor only becomes exhaust after the first getMore.
   for (int i = 0; i < 5; i++) {
      bool ok = mongoc_collection_insert_one(coll, tmp_bson("{'i': %d}", i), NULL /* opts */, NULL /* reply */, &error);
