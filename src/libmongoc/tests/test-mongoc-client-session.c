...
_test_session_pool_timeout (bool pooled)
{
    ...
    * trigger discovery
    */
   server_id = mongoc_topology_select_server_id (client->topology, MONGOC_SS_READ, NULL, NULL, &error);
...
_test_advance_operation_time (mongoc_client_session_t *cs, uint32_t t, uint32_t i, bool should_advance)
{
   ...
   if (should_advance) {
      ...
      fprintf (stderr,
               "Shouldn't have advanced from operationTime %" PRIu32 ", %" PRIu32 " to %" PRIu32 ", %" PRIu32 "\n",
               old_t,
               old_i,
               t,
               i);
   ...
   if (test->acknowledged) {
      ...
      if (test->expect_explicit_lsid) {
         if (!match_bson_with_ctx (&lsid, client_session_lsid, &ctx)) {
            fprintf (stderr, "command %s should have used client session's lsid\n", cmd_name);
            abort ();
         }
      } else {
         if (match_bson_with_ctx (&lsid, client_session_lsid, &ctx)) {
            fprintf (stderr, "command %s should not have used client session's lsid\n", cmd_name);
            abort ();
         }
      }
      ...
      if (bson_empty (&test->sent_lsid)) {
         ...
         if (!match_bson_with_ctx (&lsid, &test->sent_lsid, &ctx)) {
            fprintf (stderr, "command %s used different lsid than previous command\n", cmd_name);
            abort ();
         }
      }
   } else {
...
check_session_returned (session_test_t *test, const bson_t *lsid)
{
   ...
   if (!check_state.found) {
      fprintf (stderr, "server session %s not returned to pool\n", bson_as_json (lsid, NULL));
      abort ();
   }
}
...
#define ASSERT_OP_TIMES_EQUAL(_a, _b)                                                                      \
   if ((_a).t != (_b).t || (_a).i != (_b).i) {                                                             \
      fprintf (stderr, #_a " (%d, %d) does not match " #_b " (%d, %d)\n", (_a).t, (_a).i, (_b).t, (_b).i); \
      abort ();                                                                                            \
   }
   ...
   if (allow_read_concern) {
      ...
      for (i = 0; i < test->cmds.len; i++) {
         ...
         if (bson_has_field (cmd, "readConcern")) {
            fprintf (stderr, "Command should not have included readConcern: %s\n", bson_as_json (cmd, NULL));
            abort ();
         }
      }
   }
...
test_drop_index (session_test_t *test)
{
   ...
   r = mongoc_database_write_command_with_opts (test->session_db,
...
_test_session_dirty_helper (bool retry_succeeds)
{
   ...
   pooled_session_count_pre = mongoc_server_session_pool_size (client->topology->session_pool);
   mongoc_client_session_destroy (session);
   pooled_session_count_post = mongoc_server_session_pool_size (client->topology->session_pool);
...
test_session_install (TestSuite *suite)
{
   ...
   add_session_test_wc (suite, "/Session/watch", test_watch, true, test_framework_skip_if_not_rs_version_6);
