...
_test_session_pool_timeout(bool pooled)
{
    ...
    */
   server_id =
      mongoc_topology_select_server_id(client->topology, MONGOC_SS_READ, TEST_SS_LOG_CONTEXT, NULL, NULL, NULL, &error);
...
{
   test->succeeded =
      mongoc_collection_replace_one(test->collection, tmp_bson("{}"), tmp_bson("{}"), &test->opts, NULL, &test->error);
}
...
test_create(session_test_t *test)
{
   ...
   mongoc_database_write_command_with_opts(test->session_db, tmp_bson("{'drop': 'newname'}"), &test->opts, NULL, NULL);
