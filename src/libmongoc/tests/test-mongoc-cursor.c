...
_test_cursor_new_from_command(const char *cmd_json, const char *collection_name)
{
   ...
   cmd_cursor = mongoc_cursor_new_from_command_reply_with_opts(client, &reply, tmp_bson("{'serverId': %d}", server_id));
...
test_cursor_int64_t_maxtimems(void)
{
   ...
   bson_append_int64(max_await_time_ms, MONGOC_CURSOR_MAX_AWAIT_TIME_MS, MONGOC_CURSOR_MAX_AWAIT_TIME_MS_LEN, ms_int64);
...
_test_cursor_hint(bool pooled, bool use_primary)
{
   ...
   future = future_cursor_next(cursor, &doc);
   request = mock_rs_receives_msg(rs, MONGOC_MSG_NONE, tmp_bson("{'$db': 'test', 'find': 'test', 'filter': {'a': 1}}"));
...
test_tailable_alive(void)
{
   ...
   collection =
      mongoc_database_create_collection(database, collection_name, tmp_bson("{'capped': true, 'size': 10000}"), &error);
...
test_cursor_batchsize_override(bson_t *findopts, void (*assert_eq)(bson_iter_t *, int64_t))
{
   ...
   /* Drop and insert two documents into the collection */
   {
      ...
      ASSERT_OR_PRINT(mongoc_collection_insert_one(coll, to_insert, NULL /* opts */, NULL /* reply */, &error), error);
      ASSERT_OR_PRINT(mongoc_collection_insert_one(coll, to_insert, NULL /* opts */, NULL /* reply */, &error), error);
