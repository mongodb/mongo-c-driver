...
_test_kill_cursors (bool pooled)
{
   ...
   mock_rs_replies_to_find (request, MONGOC_QUERY_NONE, 123, 1, "db.collection", "{'b': 1}", true);
...
test_cursor_new_tailable_await (void)
{
   ...
   mock_server_replies_to_find (
      request, MONGOC_QUERY_NONE, 0 /* cursor id */, 1 /* number returned */, "db.collection", "{'_id': 1}", true);
...
_test_cursor_hint (bool pooled, bool use_primary)
{
   ...
   mock_rs_replies_to_find (request, MONGOC_QUERY_NONE, 0, 1, "test.test", "{'b': 1}", true);
   BSON_ASSERT (future_get_bool (future));
   ASSERT_MATCH (doc, "{'b': 1}");
...
test_empty_final_batch (void)
{
   ...
   mock_server_replies_to_find (request, MONGOC_QUERY_NONE, 1234, 0, "db.coll", "{}", true);
   ...
   mock_server_replies_to_find (request, MONGOC_QUERY_NONE, 1234, 0, "db.coll", "" /* empty */, true);
   ...
   mock_server_replies_to_find (request, MONGOC_QUERY_NONE, 0 /* cursor id */, 0, "db.coll", "" /* empty */, true);
