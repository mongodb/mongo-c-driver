...
test_server_selection_logic_cb (bson_t *test)
{
   ...
   mongoc_topology_description_suitable_servers (
      &selected_servers, op, &topology, read_prefs, NULL, MONGOC_TOPOLOGY_LOCAL_THRESHOLD_MS);
...
deactivate_fail_points (mongoc_client_t *client, uint32_t server_id)
{
   ...
   if (sd->type == MONGOC_SERVER_RS_PRIMARY && sd->max_wire_version >= WIRE_VERSION_RETRY_WRITES) {
...
set_uri_opts_from_bson (mongoc_uri_t *uri, const bson_t *opts)
{
   ...
   while (bson_iter_next (&iter)) {
      ...
      if (!strcmp (key, "w")) {
         ...
         MONGOC_ERROR ("Unsupported clientOptions field \"%s\" in %s", key, bson_as_json (opts, NULL));
         abort ();
      }
   }
}
...
_skip_if_unsupported (const char *test_name, bson_t *original)
{
   int i;
   bool skip = false;
   const char *unsupported_tests[] = {"/retryable_reads/gridfs-downloadByName",
   ...
   if (skip) {
      ...
      for (bson_iter_recurse (&iter, &iter); bson_iter_next (&iter);) {
         ...
         BSON_APPEND_DOCUMENT_BEGIN (&modified_tests, bson_iter_key (&iter), &modified_test);
