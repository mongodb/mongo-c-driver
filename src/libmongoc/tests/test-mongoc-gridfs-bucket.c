...
gridfs_spec_upload_operation(mongoc_database_t *db, mongoc_gridfs_bucket_t *bucket, bson_t *act, bson_t *assert)
{
   ...
   options = bson_new_from_data(bson_iter_value(&iter)->value.v_doc.data, bson_iter_value(&iter)->value.v_doc.data_len);
...
run_gridfs_spec_test(mongoc_database_t *db, mongoc_gridfs_bucket_t *bucket, bson_t *test)
{
   ...
   assert = bson_new_from_data(bson_iter_value(&iter)->value.v_doc.data, bson_iter_value(&iter)->value.v_doc.data_len);
...
test_gridfs_bucket_install(TestSuite *suite)
{
   ...
   TestSuite_AddFull(
      suite, "/gridfs/upload_error [lock:live-server]", test_upload_error, NULL, NULL, test_framework_skip_if_no_auth);
   TestSuite_AddFull(suite,
                     "/gridfs/find_w_session [lock:live-server]",
                     test_find_w_session,
                     NULL,
                     NULL,
