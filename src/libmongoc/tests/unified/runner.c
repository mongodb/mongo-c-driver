...
skipped_unified_test_t SKIPPED_TESTS[] = {
   ...
   {"backpressure-network-timeout-fail-single", SKIP_ALL_TESTS}, // Implemented in test-client-backpressure.c
   {"backpressure-network-error-fail-replicaset", SKIP_ALL_TESTS}, // Implemented in test-client-backpressure.c
{"backpressure-network-timeout-fail-replicaset", SKIP_ALL_TESTS}, // Implemented in test-client-backpressure.c
...
get_topology_type(mongoc_client_t *client)
{
   ...
   if (is_replset(&reply)) {
      ...
      cursor = mongoc_collection_find_with_opts(config_shards, tmp_bson("{}"), NULL /* opts */, NULL /* read prefs */);
...
test_check_expected_events_for_client(test_t *test, bson_t *expected_events_for_client, bson_error_t *error)
{
   ...
   if (expected_num_events != actual_num_events) {
      ...
      if (too_few_events || too_many_events) {
         test_set_error(error, "expected: %" PRIu32 " events but got %" PRIu32, expected_num_events, actual_num_events);
         goto done;
      }
   }
...
static bool
test_check_expected_log_messages_for_client(test_t *test, bson_t *expected_log_messages_for_client, bson_error_t *error)
{
   bool ret = false;
...
test_check_expected_log_messages(test_t *test, bson_error_t *error)
{
   ...
   BSON_FOREACH(test->expect_log_messages, iter)
   {
      ...
      if (!test_check_expected_log_messages_for_client(test, &expected_log_messages_for_client, error)) {
         test_diagnostics_error_info("checking expected log messages: %s", tmp_json(&expected_log_messages_for_client));
         goto done;
      }
   }
