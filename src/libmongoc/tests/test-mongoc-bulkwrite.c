...
// test_bulkwrite_bad_result_index is a regression test for CDRIVER-6369.
static void
test_bulkwrite_bad_result_index(void)
{
   mock_server_t *server = mock_server_with_auto_hello(WIRE_VERSION_8_0);
   mock_server_run(server);
   mongoc_client_t *client = mongoc_client_new_from_uri(mock_server_get_uri(server));

   const int bad_indexes[] = {-1, 123};

   for (size_t i = 0; i < sizeof(bad_indexes) / sizeof(bad_indexes[0]); i++) {
      const int bad_index = bad_indexes[i];
      mongoc_bulkwrite_t *bw = mongoc_client_bulkwrite_new(client);
      ...
      future_t *fut = future_bulkwrite_execute(bw, NULL);
      request_t *req = mock_server_receives_msg(server,
                                                MONGOC_MSG_NONE,
                                                tmp_bson("{'bulkWrite': 1 }"),
                                                tmp_bson("{'ns': 'db.coll'}"), // "nsInfo"
                                                tmp_bson("{'update': 0}"));    // "ops"

      char *reply_str = bson_strdup_printf(
         BSON_STR({
            "ok" : 1,
            "nInserted" : 0,
            "nMatched" : 0,
            ...
            "cursor" : {
               "id" : 0,
               // "idx" refers to non-existent operation.
               "firstBatch" : [ {"ok" : 1, "idx" : {"$numberLong" : "%d"}, "n" : 0, "nModified" : 0} ],
               "ns" : "admin.$cmd.bulkWrite"
            }
         }),
         bad_index);
      reply_to_request_simple(req, reply_str);
      mongoc_bulkwritereturn_t bwr = future_get_mongoc_bulkwritereturn_t(fut);
...
test_bulkwrite_install(TestSuite *suite)
{
   ...
   TestSuite_AddMockServerTest(suite, "/bulkwrite/bad_result_index", test_bulkwrite_bad_result_index);
}
