...
test_update_arrayfilters(void *ctx)
{
   ...
   for (i = 1; i < 4; i++) {
      ret =
         mongoc_bulk_operation_insert_with_opts(bulk, tmp_bson("{'_id': %d, 'a': [{'x':1}, {'x':2}]}", i), NULL, &err);
...
test_replace_one_with_opts_validate(void)
{
   ...
   BSON_ASSERT(!mongoc_bulk_operation_replace_one_with_opts(bulk, tmp_bson("{}"), tmp_bson("{'$a': 1}"), NULL, &error));
   ASSERT_ERROR_CONTAINS(
...
test_large_inserts_ordered(void *ctx)
{
   ...
   huge_doc = BCON_NEW("a", BCON_INT32(1));
   bson_append_utf8(huge_doc, "long-key-to-make-this-fail", -1, huge_string(client), (int)huge_string_length(client));
...
_test_bulk_hint(bool pooled, bool use_primary)
{
   ...
   request =
      mock_rs_receives_msg(rs, MONGOC_MSG_NONE, tmp_bson("{'$db': 'test', 'insert': 'test'}"), tmp_bson("{'_id': 1}"));
...
test_bulk_invalid_write_concern(void)
{
   ...
   bulk =
      mongoc_collection_create_bulk_operation_with_opts(collection, tmp_bson("{'writeConcern': {'w': 0, 'j': true}}"));
   BSON_ASSERT(!mongoc_bulk_operation_insert_with_opts(bulk, tmp_bson("{}"), NULL, &error));
   ASSERT_ERROR_CONTAINS(
...
_test_bulk_collation(bool w, bulkop op)
{
   ...
   if (w) {
      request =
         mock_server_receives_msg(mock_server, MONGOC_MSG_NONE, tmp_bson(expect_msg, w ? 1 : 0), tmp_bson(expect_doc));
...
test_bulk_bypass_document_validation(void)
{
   ...
   bulk = mongoc_collection_create_bulk_operation_with_opts(collection, tmp_bson("{'bypassDocumentValidation': true}"));
