...
#define DESTROY_CHANGE_STREAM(cursor_id)                                                                              \
   do {                                                                                                               \
      ...
      mock_server_replies_simple (_request, "{ 'cursorsKilled': [ " #cursor_id " ] }");                               \
      future_wait (_future);                                                                                          \
...
test_change_stream_resumable_error (void)
{
   ...
   future = future_change_stream_next (stream, &next_doc);
   request = mock_server_receives_msg (server, MONGOC_MSG_NONE, tmp_bson (expected_msg, 123));
   mock_server_replies_simple (request, "{ 'code': 10107, 'errmsg': 'not primary', 'ok': 0 }");
   request_destroy (request);

   /* Retry command */
   request = mock_server_receives_msg (server, MONGOC_MSG_NONE, watch_cmd);
   mock_server_replies_simple (request, "{'code': 123, 'errmsg': 'bad cmd', 'ok': 0}");
   request_destroy (request);
...
test_change_stream_live_watch (void *test_ctx)
{
   ...
   ASSERT_OR_PRINT (mongoc_collection_update (
                       coll, MONGOC_UPDATE_NONE, tmp_bson ("{}"), tmp_bson ("{'$set': {'x': 'z'} }"), wc, &err),
                    err);
...
static bool
change_stream_spec_operation_cb (json_test_ctx_t *ctx, const bson_t *test, const bson_t *operation)
{
   bson_t reply;
   bool res;

   mongoc_collection_t *coll = mongoc_client_get_collection (
      ctx->client, bson_lookup_utf8 (operation, "database"), bson_lookup_utf8 (operation, "collection"));
...
static void
change_stream_spec_before_test_cb (json_test_ctx_t *test_ctx, const bson_t *test)
{
   change_stream_spec_ctx_t *ctx = (change_stream_spec_ctx_t *) test_ctx->config->ctx;
   bson_t opts;
   bson_t pipeline;
   bson_t tmp;
   ...
   if (!strcmp (target, "collection")) {
      ...
      ASSERT_WITH_MSG (false, "target unknown: \"%s\" in test: %s", target, bson_as_json (test, NULL));
   }
   bson_destroy (&opts);
}
                               ...
                               const bson_t **error_reply)
{
   ...
   do {
      ...
      has_error = mongoc_change_stream_error_document (stream, error, error_reply);
...
change_stream_spec_after_test_cb (json_test_ctx_t *test_ctx, const bson_t *test)
{
   ...
   if (bson_has_field (test, "result.error")) {
      ...
      ret = _iterate_until_error_or_event (ctx->change_stream, &doc, &error, &reply);
      if (ret) {
         test_error ("Expected error, but change stream did not return an error");
      }
              ...
              const char *cursor_pbr)
{
   ...
   mock_server_replies_simple (request, "{'cursor': {'id': 0,'ns': 'db.coll','firstBatch': []},'ok': 1 }");
   request_destroy (request);
...
test_change_stream_install (TestSuite *suite)
{
   ...
   install_json_test_suite (suite, JSON_DIR, "/change_streams/legacy", &test_change_stream_spec_cb);
}
