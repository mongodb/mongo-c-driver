...
test_sasl_canon_user_longest_accepted (void)
{
   ...
   ASSERT_CMPINT (
      _mongoc_cyrus_canon_user (NULL, NULL, in, inlen, 0u, NULL, out, CANON_OUT_MAX, &out_len), ==, SASL_OK);
...
test_sasl_canon_user_too_large (void)
{
   ...
   for (size_t i = 0u; i < sizeof oversized / sizeof *oversized; i++) {
      ...
      ASSERT_WITH_MSG (_mongoc_cyrus_canon_user (
                          NULL, NULL, in, oversized[i], 0u, NULL, out, CANON_OUT_MAX, &out_len) == SASL_BUFOVER,
