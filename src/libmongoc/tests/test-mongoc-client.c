...
test_mongoc_client_authenticate_cached (bool pooled)
{
   ...
   if (pooled) {
      ASSERT_CAPTURED_LOG ("The cachekey broke", MONGOC_LOG_LEVEL_WARNING, "Failed authentication");
   }
   capture_logs (false);
...
test_mongoc_client_authenticate_failure (void *context)
{
    ...
    * same sort of errors.
    */
   r = mongoc_collection_update (collection, MONGOC_UPDATE_NONE, &q, &empty, NULL, &error);
   BSON_ASSERT (!r);
   ASSERT_CMPINT (error.domain, ==, MONGOC_ERROR_CLIENT);
   ASSERT_CMPINT (error.code, ==, MONGOC_ERROR_CLIENT_AUTHENTICATE);
...
test_get_database_names (void)
{
   ...
   mock_server_replies (request, 0, 0, 0, 1, "{'ok': 1.0, 'databases': [{'name': 'a'}, {'name': 'local'}]}");
   ...
   request = mock_server_receives_msg (
      server, MONGOC_MSG_NONE, tmp_bson ("{'$db': 'admin', 'listDatabases': 1, 'nameOnly': true}"));
   mock_server_replies (request, 0, 0, 0, 1, "{'ok': 0.0, 'code': 17, 'errmsg': 'err'}");
...
_test_mongoc_client_select_server (bool pooled)
{
   ...
   ASSERT (!strcmp (server_type, "Standalone") || !strcmp (server_type, "RSPrimary") ||
   ...
   ASSERT (sd);
   server_type = mongoc_server_description_type (sd);
   ASSERT (!strcmp (server_type, "Standalone") || !strcmp (server_type, "RSSecondary") ||
           !strcmp (server_type, "Mongos"));
...
_test_mongoc_client_select_server_error (bool pooled)
{
   ...
   if (tdtype == MONGOC_TOPOLOGY_SINGLE || tdtype == MONGOC_TOPOLOGY_SHARDED) {
      ASSERT (sd);
      server_type = mongoc_server_description_type (sd);
      ASSERT (!strcmp (server_type, "Standalone") || !strcmp (server_type, "Mongos"));
      mongoc_server_description_destroy (sd);
   } else {
      ASSERT (!sd);
      ASSERT_CMPINT (error.domain, ==, MONGOC_ERROR_SERVER_SELECTION);
