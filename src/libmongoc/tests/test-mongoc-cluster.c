...
server_id_for_reads (mongoc_cluster_t *cluster)
{
   ...
   server_stream = mongoc_cluster_stream_for_reads (cluster, NULL, NULL, NULL, false, &error);
...
replies_with_cluster_time (request_t *request, int t, int i, const char *docs_json)
{
   ...
   mock_server_reply_multi (request, MONGOC_REPLY_NONE, &doc, 1, 0 /* cursor id */);
...
receives_with_cluster_time (mock_server_t *server, uint32_t timestamp, uint32_t increment, bson_t *command)
{
   ...
   if (t != timestamp || i != increment) {
      fprintf (stderr, "Expected Timestamp(%d, %d), got Timestamp(%d, %d)\n", timestamp, increment, t, i);
      abort ();
   }
...
future_command_private (mongoc_client_t *client)
{
   ...
   server_stream = mongoc_cluster_stream_for_writes (&client->cluster, NULL, NULL, &error);
   ASSERT_OR_PRINT (server_stream, error);
...
_test_dollar_query (void *ctx)
{
   ...
   mock_server_replies_to_find (request, MONGOC_QUERY_NONE, 0, 0, "db.collection", "", true);
