...
test_topology_client_creation (void)
{
   ...
   server_stream = mongoc_cluster_stream_for_reads (&client_a->cluster, NULL, NULL, NULL, false, &error);
...
_test_topology_invalidate_server (bool pooled)
{
   ...
   /* call explicitly */
   server_stream = mongoc_cluster_stream_for_reads (&client->cluster, NULL, NULL, NULL, false, &error);
   ASSERT_OR_PRINT (server_stream, error);
   sd = server_stream->sd;
...
test_topology_install (TestSuite *suite)
{
   ...
   _TestSuite_AddMockServerTest (
      suite, "/Topology/try_once/succeed", test_select_after_try_once, test_framework_skip_if_time_sensitive, NULL);
