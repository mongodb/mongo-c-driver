...
_get_gridfs(mock_server_t *server, mongoc_client_t *client, mongoc_query_flags_t flags)
{
   ...
   request = mock_server_receives_msg(server, MONGOC_MSG_NONE, tmp_bson("{'$db': 'db', 'createIndexes': 'fs.chunks'}"));
   ...
   request = mock_server_receives_msg(server, MONGOC_MSG_NONE, tmp_bson("{'$db': 'db', 'createIndexes': 'fs.files'}"));
...
test_remove_operator_injection(void)
{
   ...
   {
      int64_t count =
         mongoc_collection_count_documents(mongoc_gridfs_get_files(gridfs), tmp_bson("{}"), NULL, NULL, NULL, &error);
...
test_properties(void)
{
   ...
   ASSERT(
      BCON_EXTRACT((bson_t *)mongoc_gridfs_file_get_aliases(file), "0", BCONE_UTF8(alias0), "1", BCONE_UTF8(alias1)));
...
_check_chunk_count(mongoc_gridfs_t *gridfs, int64_t len, int64_t chunk_size)
{
   ...
   cnt = mongoc_collection_count_documents(mongoc_gridfs_get_chunks(gridfs), tmp_bson(NULL), NULL, NULL, NULL, &error);
...
test_missing_chunk(void *ctx)
{
   ...
   for (;;) {
      ...
      if (r > 0) {
         ...
         ASSERT_ERROR_CONTAINS(error, MONGOC_ERROR_GRIDFS, MONGOC_ERROR_GRIDFS_CHUNK_MISSING, "missing chunk number 1");
