...
test_topology_scanner_discovery(void)
{
   ...
   future = future_topology_select(client->topology, MONGOC_SS_READ, TEST_SS_LOG_CONTEXT, secondary_pref, NULL, &error);
...
test_topology_scanner_socket_timeout(void)
{
   ...
   expected_msg = bson_strdup_printf("socket timeout calling hello on '%s'", mongoc_uri_get_hosts(uri)->host_and_port);
