...
test_aggregate_w_write_concern(void *ctx)
{
   ...
   ASSERT_ERROR_CONTAINS(cursor->error, MONGOC_ERROR_COMMAND, MONGOC_ERROR_COMMAND_INVALID_ARG, "Invalid writeConcern");
   bad_wc->wtimeout = 0;
...
test_insert_many(void)
{
   ...
   ASSERT_OR_PRINT(mongoc_collection_insert_many(collection, (const bson_t **)bptr, 10, NULL, &reply, &error), error);
...
test_index_geo(void *unused)
{
   ...
   description = mongoc_topology_description_server_by_id_const(mc_tpld_unsafe_get_const(client->topology), id, &error);
...
test_estimated_document_count(void)
{
   ...
   future =
      future_collection_estimated_document_count(collection, tmp_bson("{'limit': 2, 'skip': 1}"), NULL, &reply, &error);
...
test_find_and_modify_write_concern(void)
{
   ...
   future = future_collection_find_and_modify(collection, &doc, NULL, update, NULL, false, false, true, &reply, &error);
...
test_many_return(void)
{
   ...
   ASSERT_OR_PRINT(
      mongoc_collection_insert_many(collection, (const bson_t **)docs, (uint32_t)N_BSONS, NULL, NULL, &error), error);
...
test_find_limit(void)
{
   ...
   cursor =
      mongoc_collection_find_with_opts(collection, tmp_bson("{}"), tmp_bson("{'limit': {'$numberLong': '2'}}"), NULL);
...
test_find_indexes_err(void)
{
   ...
   request = mock_server_receives_msg(server, MONGOC_MSG_NONE, tmp_bson("{'$db': 'db', 'listIndexes': 'collection'}"));
...
_test_update_and_replace(bool is_replace, bool is_multi)
{
   ...
   if (is_replace) {
      ...
      ret =
         mongoc_collection_insert_one(coll, tmp_bson("{'_id': 6, 'a': [{'x':1},{'x':2}]}"), &opts_with_wc, NULL, &err);

      ASSERT_OR_PRINT(ret, err);

      update = tmp_bson("{'$set': {'a.$[i].x': 3}}");
      ret = fn(coll, tmp_bson("{'_id': 6}"), update, tmp_bson("{'arrayFilters': [{'i.x': {'$gt': 1}}]}"), &reply, &err);
...
_test_delete_one_or_many(bool is_multi)
{
   ...
   ret =
      fn(coll, tmp_bson("{'x': 'PING'}"), tmp_bson("{'collation': {'locale': 'en_US', 'strength': 2 }}"), &reply, &err);
...
_test_update_or_replace_with_collation(bool is_replace, bool is_multi)
{
   ...
   client = test_framework_client_new_from_uri(mock_server_get_uri(server), NULL);
   collection = mongoc_client_get_collection(client, "db", "collection");
   future = fn(collection, tmp_bson("{}"), tmp_bson("{}"), tmp_bson("{'collation': {'locale': 'en'}}"), NULL, &error);
...
test_remove_multi(void)
{
   ...
   ASSERT_OR_PRINT(
      mongoc_collection_remove(collection, MONGOC_REMOVE_NONE, tmp_bson("{'_id': {'$gte': 8}}"), NULL, &error), error);
