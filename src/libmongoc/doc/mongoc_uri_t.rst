...
MONGOC_URI_ENABLEOVERLOADRETARGETING       enableoverloadretargeting         false                             If "true", the driver deprioritizes a server that returns a `SystemOverloadedError` when selecting a server for retry.
