...
MONGOC_URI_MAXADAPTIVERETRIES              maxadaptiveretries                2                                 The maximum number of times the driver retries an operation that fails with a `SystemOverloadedError`. Must be a non-negative integer.
MONGOC_URI_ENABLEOVERLOADRETARGETING       enableoverloadretargeting         false                             If "true", the driver deprioritizes servers which returned a `SystemOverloadedError` when selecting a server to retry an operation.
