...
MONGOC_URI_MAXADAPTIVERETRIES              maxadaptiveretries                2                                 The maximum number of times the driver retries an operation that fails with a `SystemOverloadedError`. Must be a non-negative integer. This option works with MongoDB Atlas Server Version 9.0 and above.
MONGOC_URI_ENABLEOVERLOADRETARGETING       enableoverloadretargeting         false                             If "true", the driver deprioritizes servers which returned a `SystemOverloadedError` when selecting a server to retry an operation. This option works with MongoDB Atlas Server Version 9.0 and above.
