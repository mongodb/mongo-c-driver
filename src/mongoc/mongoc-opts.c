   ...
   mongoc_collection_t *collection)
{
   ...
   mongoc_insert_one_opts->crud.writeConcern = mongoc_write_concern_copy(collection->write_concern);
   mongoc_insert_one_opts->crud.client_session = NULL;
   mongoc_insert_one_opts->crud.bypassDocumentValidation = MONGOC_BYPASS_DOCUMENT_VALIDATION_DEFAULT;
      ...
      while (bson_iter_next (&iter)) {
         if (!strcmp (bson_iter_key (&iter), "writeConcern")) {
            ...
            if (!_mongoc_convert_session_id (
               &iter, mongoc_insert_one_opts->crud.client_session, error, collection)) {
               return false;
            }
         }
            ...
            if (!_mongoc_convert_bypass_doc_eval(
   ...
   mongoc_collection_t *collection)
{
   ...
   while (bson_iter_next (&iter)) {
      if (!strcmp (bson_iter_key (&iter), "writeConcern")) {
         ...
         if (!_mongoc_convert_session_id (
               &iter, mongoc_insert_many_opts->crud.client_session, error, collection)) {
            return false;
         }
      }
      else if (!strcmp (bson_iter_key (&iter), "validate")) {
