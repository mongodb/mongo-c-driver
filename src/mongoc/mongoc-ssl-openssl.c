...
static void _mongoc_ssl_openssl_thread_startup(void);
static void _mongoc_ssl_openssl_thread_cleanup(void);

/*
 *-------------------------------------------------------------------------
 *
 * _mongoc_ssl_openssl_init --
 *
 *       Initialization function for OpenSSL SSL.
 *
...
_mongoc_ssl_openssl_init (void)
{
   ...
   _mongoc_ssl_openssl_thread_startup ();
...
{
   _mongoc_ssl_openssl_thread_cleanup ();
}

/*
 *-------------------------------------------------------------------------
 *
 * _mongoc_ssl_openssl_password_cb --
 *
 *-------------------------------------------------------------------------
 */

static int
_mongoc_ssl_openssl_password_cb (char *buf,
                         int   num,
                         int   rwflag,
                         void *user_data)
{
 ...
 */
static bool
_mongoc_ssl_openssl_hostcheck (const char *pattern,
                               const char *hostname)
{
 ...
 *-------------------------------------------------------------------------
 *
 * _mongoc_ssl_openssl_check_cert --
 *
            ...
            if (name->type == target) {
               ...
               case GEN_DNS:
                  ...
                  if ((length == bson_strnlen (check, length)) &&
                      _mongoc_ssl_openssl_hostcheck (check, host)) {
                     r = 1;
                  }
 ...
 *-------------------------------------------------------------------------
 *
 * _mongoc_ssl_openssl_setup_ca --
 *
 *-------------------------------------------------------------------------
 */

static bool
_mongoc_ssl_openssl_setup_ca (SSL_CTX    *ctx,
                              const char *cert,
                              const char *cert_dir)
{
   BSON_ASSERT(ctx);
 ...
 *-------------------------------------------------------------------------
 *
 * _mongoc_ssl_openssl_setup_crl --
 *
 *-------------------------------------------------------------------------
 */

static bool
_mongoc_ssl_openssl_setup_crl (SSL_CTX    *ctx,
                               const char *crlfile)
{
   X509_STORE *store;
   X509_LOOKUP *lookup;
 ...
 *-------------------------------------------------------------------------
 *
 * _mongoc_ssl_openssl_setup_pem_file --
 *
 *-------------------------------------------------------------------------
 */

static bool
_mongoc_ssl_openssl_setup_pem_file (SSL_CTX    *ctx,
                                    const char *pem_file,
                                    const char *password)
{
   ...
   if (password) {
      ...
      SSL_CTX_set_default_passwd_cb (ctx, _mongoc_ssl_openssl_password_cb);
   }
 ...
 *-------------------------------------------------------------------------
 *
 * _mongoc_ssl_openssl_ctx_new --
 *
 *       Create a new ssl context declaratively
 *
 *       The opt.pem_pwd parameter, if passed, must exist for the life of
 *       this context object (for storing and loading the associated pem
 *       file).
 *
 *-------------------------------------------------------------------------
 */
...
_mongoc_ssl_openssl_ctx_new (mongoc_ssl_opt_t *opt)
{
   ...
   if ((!opt->pem_file ||
        _mongoc_ssl_openssl_setup_pem_file (ctx, opt->pem_file, opt->pem_pwd))
       && (!(opt->ca_file ||
             opt->ca_dir) ||
           _mongoc_ssl_openssl_setup_ca (ctx, opt->ca_file, opt->ca_dir))
       && (!opt->crl_file || _mongoc_ssl_openssl_setup_crl (ctx, opt->crl_file))
       ) {
      return ctx;
   } else {
      SSL_CTX_free (ctx);
 ...
 *-------------------------------------------------------------------------
 *
 * _mongoc_ssl_openssl_extract_subject --
 *
 *-------------------------------------------------------------------------
 */
...
_mongoc_ssl_openssl_thread_id_callback (void)
{
   unsigned long ret;
...
static void
_mongoc_ssl_openssl_thread_locking_callback (int         mode,
                                             int         type,
                                             const char *file,
                                             int         line)
{
...
static void
_mongoc_ssl_openssl_thread_startup (void)
{
   int i;
   ...
   CRYPTO_set_locking_callback (_mongoc_ssl_openssl_thread_locking_callback);
   CRYPTO_set_id_callback (_mongoc_ssl_openssl_thread_id_callback);
}

static void
_mongoc_ssl_openssl_thread_cleanup (void)
{
   int i;
