                           ...
                           char *str)                    /* OUT */
{
   ...
   low = (uint32_t) dec->low, midl = (uint32_t) (dec->low >> 32), midh = (uint32_t) dec->high,
      ...
      if (exponent >= 0) {
         ...
         *str_out = '\0';
         ...
         if (radix_position > 0) { /* non-zero digits before radix */
            for (i = 0; i < radix_position && (str_out - str) < BSON_DECIMAL128_STRING; i++) {
               *(str_out++) = *(significand_read++) + '0';
            }
         ...
         *(str_out++) = '.';
         ...
         for (i = 0;
              (i < significand_digits - BSON_MAX (radix_position - 1, 0)) && (str_out - str) < BSON_DECIMAL128_STRING;
              i++) {
            *(str_out++) = *(significand_read++) + '0';
         }
         *str_out = '\0';
