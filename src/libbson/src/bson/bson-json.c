...
_bson_json_read_start_map (bson_json_reader_t *reader) /* IN */
{
   ...
   if (bson->read_state == BSON_JSON_IN_BSON_TYPE) {
      ...
      case BSON_JSON_LF_TYPE:
         ...
         STACK_PUSH_DOC (bson_append_document_begin (STACK_BSON_PARENT, key, len, STACK_BSON_CHILD));
...
_bson_json_read_code_or_scope_key (bson_json_reader_bson_t *bson, bool is_scope, const uint8_t *val, size_t len)
{
   ...
   if (code->in_scope) {
      ...
      if (!bson->code_data.key_buf.len) {
         ...
         _bson_json_buf_set (&bson->code_data.key_buf, bson->key_buf.buf, bson->key_buf.len);
      }
                         ...
                         size_t len)                 /* IN */
{
   ...
   if (bson->read_state == BSON_JSON_IN_BSON_TYPE) {
      ...
      else if (!strcmp ("$dbPointer", (const char *) val))
      {
         ...
         _bson_json_buf_set (&bson->dbpointer_key, bson->key_buf.buf, bson->key_buf.len);
