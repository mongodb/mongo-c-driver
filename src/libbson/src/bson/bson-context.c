...
_bson_context_set_oid_seq64 (bson_context_t *context, /* IN */
                             bson_oid_t *oid)         /* OUT */
{
   uint64_t seq = (uint64_t) mcd_atomic_int64_fetch_add ((int64_t *) &context->seq64, 1, mcommon_memory_order_seq_cst);
...
_bson_context_init_random (bson_context_t *context, bool init_seq)
{
   ...
   rand_params.rand_call_counter = mcd_atomic_int64_fetch_add (&s_rand_call_counter, 1, mcommon_memory_order_seq_cst);
