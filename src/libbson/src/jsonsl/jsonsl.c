...
#define X(m)                                                        \
   printf ("\t%-30s %20lu (%0.2f%%)\n",                             \
           ...
           (float) ((float) (GlobalMetrics.metric_##m /             \
                             (float) GlobalMetrics.metric_TOTAL)) * \
              100);
   XMETRICS
#undef X
...
jsonsl_new (int nlevels)
{
   ...
   jsn = (struct jsonsl_st *) bson_malloc0 (
 ...
 * @param[in,out] bytes_p A pointer to the current buffer (i.e. current
 * position)
...
#define STACK_PUSH                                                      \
   if (jsn->level >= (levels_max - 1)) {                                \
      jsn->error_callback (                                             \
         jsn, JSONSL_ERROR_LEVELS_EXCEEDED, state, (char *) c);         \
      return;                                                           \
   }                                                                    \
...
#define DO_CALLBACK(T, action)                                        \
   if (jsn->call_##T && jsn->max_callback_level > state->level &&     \
       state->ignore_callback == 0) {                                 \
      if (jsn->action_callback_##action) {                            \
         jsn->action_callback_##action (                              \
            jsn, JSONSL_ACTION_##action, state, (jsonsl_char_t *) c); \
...
#define CONTINUE_NEXT_CHAR() continue
   ...
   GT_AGAIN:
      ...
      if (state_type & JSONSL_Tf_STRINGY) {
         ...
         if (IS_NORMAL_NUMBER) {
            ...
            if (CUR_CHAR == '0') {
               state->special_flags =
                  JSONSL_SPECIALf_ZERO | JSONSL_SPECIALf_SIGNED;
   ...
   GT_STRUCTURAL_TOKEN:
      ...
      GT_SPECIAL_BEGIN:
          ...
          * we are not doing full numerical/value decoding anyway (but only
          * hinting), we only check upon entry.
          */
         if (state->type != JSONSL_T_SPECIAL) {
            int special_flags = extract_special (CUR_CHAR);
            if (!special_flags) {
               /**
                * Try to do some heuristics here anyway to figure out what kind
                * of error this is. The 'special' case is a fallback scenario
                * anyway.
                */
               if (CUR_CHAR == '\0') {
...
#define JPR_BAIL(err) \
   *errp = err;       \
   ...
   components =
      (struct jsonsl_jpr_component_st *) malloc (sizeof (*components) * count);
   if (!components) {
...
jsonsl_jpr_match_state_init (jsonsl_t jsn, jsonsl_jpr_t *jprs, size_t njprs)
{
   ...
   jsn->jpr_count = njprs;
   jsn->jpr_root =
      (size_t *) calloc (1, sizeof (size_t) * njprs * jsn->levels_max);
 ...
 *
 * Since the parent is checked for a match as well, we maintain a 'serial'
 * counter. Whenever we traverse an element, we expect the serial to be the same
 * as a global integer. If they do not match, we re-initialize the context, and
 * set the serial.
 *
   ...
   *jmptable = 0;
   ...
   for (ii = 0; ii < jsn->jpr_count; ii++) {
      ...
      if (jmp_cur) {
         ...
         *out = jsonsl_jpr_match (
            jpr, parent_state->type, parent_state->level, key, nkey);
   ...
   *err = JSONSL_ERROR_##e;                               \
   ...
   for (; len; len--, c++, out++) {
      ...
      if ((toEscape && toEscape[(unsigned char) c[1] & 0x7f] == 0 &&
           c[1] != '\\' && c[1] != '"')) {
         /* if we don't want to unescape this string, write the escape sequence
          * to the output */
         *out++ = *c++;
         --len;
         goto GT_ASSIGN;
...
/**
 * Contains characters which signal the termination of any of the 'special'
 * bareword values.
 */
