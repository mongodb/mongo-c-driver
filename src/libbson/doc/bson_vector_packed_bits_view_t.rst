:man_page: bson_vector_packed_bits_view_t

bson_vector_packed_bits_view_t
==============================

A reference to mutable non-owned BSON Binary data holding a valid Vector of ``packed_bits`` element type.
...
:symbol:`bson_vector_packed_bits_view_t` is a structure that acts as an opaque reference to a block of memory that has been validated as a ``packed_bits`` vector.
...
.. only:: html
  ...
  .. toctree::
    ...
    bson_vector_packed_bits_view_init
...
.. seealso::
  ...
  | :symbol:`bson_append_vector_packed_bits`
  | :symbol:`bson_vector_packed_bits_const_view_t`
