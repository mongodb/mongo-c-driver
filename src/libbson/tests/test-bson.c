...
bloat(bson_t *b)
{
   ...
   /* spilled over */
   ASSERT(!(b->flags & BSON_FLAG_INLINE_DATA));
}
...
test_bson_dsl_parse(void)
{
   ...
   found = false;
   bsonParse(
      *TMP_BSON_FROM_JSON({"foo" : null, "bar" : null}), find(key("foo"), halt), find(key("bar"), do(found = true)));
   BSON_ASSERT(!bsonParseError);
   BSON_ASSERT(!found);
   ...
   b = 0;
   bsonParse(
      *TMP_BSON_FROM_JSON({"foo" : 1, "bar" : 2}),
      if (a == 812, then(find(key("foo"), do(b = bsonAs(int32)))), else(find(key("bar"), do(b = bsonAs(int32))))));
   ASSERT_CMPINT(b, ==, 1);
   a = 4;
   bsonParse(
      *TMP_BSON_FROM_JSON({"foo" : 1, "bar" : 2}),
...
test_bson_dsl_predicate(void)
{
   ...
   bsonParse( //
      *document,
      ...
      require(
         key("doc"), require(type(doc)), require(not(empty)), visitEach(require(key("hello")), require(type(null)))),
