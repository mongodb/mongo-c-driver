...
test_bson_json_read_buffering(void)
{
   ...
   for (n_docs = 1; n_docs < 10; n_docs++) {
      ...
      for (i = 0; i < 50; i++) {
         ...
         reader =
            bson_json_data_reader_new(true /* "allow_multiple" is unused */, (size_t)RAND_R(&seed) % 100 /* bufsize*/);
...
test_bson_json_dbref(void)
{
   ...
   bson_t *bson_with_db =
      BCON_NEW("key", "{", "$ref", BCON_UTF8("collection"), "$id", BCON_INT32(1), "$db", BCON_UTF8("database"), "}");
...
test_bson_json_date(void)
{
   ...
   test_bson_json_date_error("{ \"dt\" : { \"$date\" : \"1970-01-01T01:00:00.+01:00\" } }", "milliseconds is required");
   ...
   test_bson_json_date_error("{ \"dt\" : { \"$date\" : \"1970-01-01T00:foo:00.000Z\" } }", "minute must be an integer");
...
test_bson_json_date_numberlong(void)
{
   ...
   test_bson_json_date_check("{ \"dt\" : { \"$date\" : { \"$numberLong\" : \"-62135593139000\" } } }", -62135593139000);
...
test_bson_as_json_with_opts_utf8_truncation(void)
{
   ...
   _test_bson_json_utf8_truncation(
      BCON_NEW("doc", "{", "a", BCON_UTF8("b"), "}"), BSON_JSON_MODE_CANONICAL, "{ \"doc\" : { \"a\" : \"b\" } }", -1);
...
#define ASSERT_AS_JSON(bson, mode, expect_single_quoted)                         \
   ...
   // Test other cases referenced in the Extended JSON specification that sometimes encode differently.
   {
      // Code with scope
      {
         ...
         ASSERT_AS_JSON(b, BSON_JSON_MODE_RELAXED, "{ 'codewscope' : { '$code' : 'code', '$scope' : { 'a' : 'b' } } }");
      ...
      // Date before year 1970.
      {
         ...
         ASSERT_AS_JSON(b, BSON_JSON_MODE_CANONICAL, "{ 'date_before_1970' : { '$date' : { '$numberLong' : '-1' } } }");
