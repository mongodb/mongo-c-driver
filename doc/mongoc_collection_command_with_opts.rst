...
Read preferences are applied from ``read_prefs`` or else from ``collection``. Collation, read concern, and write concern are applied from ``opts`` (:ref:`see example for the "distinct" command with opts <mongoc_client_read_command_with_opts_example>`). Read concern and collation both require MongoDB 3.2 or later, otherwise an error is returned. The write concern is ignored for MongoDB before 3.4.
