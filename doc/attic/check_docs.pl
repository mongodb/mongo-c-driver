...
my $bson_info = `pkg-config --cflags libbson-1.0`;
chomp $bson_info;

my @protos = `find mongoc -name "*.[ch]" | grep -v win32 | grep -v private | xargs cproto -Imongoc $bson_info -DMONGOC_OS_UNIX -DMONGOC_COMPILATION -i | grep -v "^/" | sort | uniq | indent -l1000 -linux`;
